...
const removeCartProduct = async (li, id) => {
  ...
  const detals = await fetchProduct(id);
  tagPrice.innerText = `${(parseFloat(tagPrice.textContent) - detals.price).toFixed(2)}`;
  if (tagPrice.textContent === '0.00') {
    tagPrice.innerText = '0';
  }
};
